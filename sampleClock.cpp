...
void ClockProblem::setValues(int timeFormat, int initialTime,int currentTime, vector<int> increments) {
...
vector<vector<int> > ClockProblem::getNextNodes(vector<int> nodeN) {
    ...
    for(int i=0; i<increments.size();i++)
    {
        ...
        else if(((nodeN.front()) + increments[i]) == 0)
...
void ClockProblem::displayOutput(vector <int> finalOutput) {
    ...
    visitedNodeList.push_front(finalOutput.front());
